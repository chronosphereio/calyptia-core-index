...
AWS_INDEX_FILE=${AWS_INDEX_FILE:-$SCRIPT_DIR/../operator.aws.index.json}
...
do
    ...
    aws ec2 describe-tags --no-paginate --region "$aws_region" \
        --filter "Name=key,Values=$IMAGE_KEY" --query 'Tags[*]' --output json | jq -cr 'unique_by(.Key,.Value)|.[].Value' > "$aws_region".tags
